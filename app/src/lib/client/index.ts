...
async function onField (arg : any) {
   console.log('field guide')
   const can : HTMLCanvasElement = document.getElementById('can')
   const dpr = window.devicePixelRatio || 1
...
webSocket.onmessage = async (event : any) => {
   ...
   if (data.action === 'field') {
      await onField( { ratio : data.ratio })
      send({ action : 'field' })
   }
     if (data.action === 'meter') {
      await onMeter()
      send({ action : 'meter' })
   }
