...
async function onImage (arg: McopyRequest) {
   return setImage(arg.image)
}

async function onMeter (arg : McopyRequest ) {
   console.log('meter')
...
async function onBlank (arg : McopyRequest ) {
   console.log('blank')
   const body = document.querySelector('body')
   body.className = ''
...
async function onFocus (arg : McopyRequest ) {
   console.log('focus')
...
async function drawFocus (can : HTMLCanvasElement, ctx : CanvasRenderingContext2D) {
   const count = 20
...
async function onField (arg : McopyRequest) {
...
// draw a field guide
async function drawField (can : HTMLCanvasElement, ctx : CanvasRenderingContext2D) {
   const count = 20
   const half = Math.round(count / 2)
   const dpr = window.devicePixelRatio || 1
   const w = can.width / dpr
...
async function onPing (arg : McopyRequest) {
   //console.log('ping')
   return true
}

function send (obj : McopyResponse) {
   if (ws) {
...
let actions : any = {
   mcopy : async (arg : McopyRequest) => { console.log('Connected'); return true; },
   image : onImage,
   field : onField,
   meter : onMeter,
   focus : onFocus,
...
ws.onmessage = async (event : any) => {
   const req : McopyRequest = JSON.parse(event.data);
   let res : McopyResponse;

   if (typeof actions[req.action] !== 'undefined') {
      try {
         await actions[req.action](req)
      } catch (err) {
         console.error(err)
      }
      res = {
